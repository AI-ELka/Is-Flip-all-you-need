...
def plot_cta_vs_pta_mean_var(df, save_dir=None):
    ...
    plt.title(f"{dataset_title} – {df.iloc[0]['aggregator'].upper()} (mean ± std), gen non federated and train federated")
...
def plot_cta_vs_pta_multi_aggregators(
    ...
    for i, (agg, df) in enumerate(dfs_by_agg.items()):
        ...
        agg_label = f"multikrum with m = {m_krum}" if agg.lower() == "krum" else f"{agg.capitalize()} aggregation"
    ...
    plt.title(f"{dataset_title}: Clean vs Poisoned Test Accuracy,  gen_labels non federated and train federated", fontsize=12)
